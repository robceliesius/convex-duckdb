    ...
    try {
      ...
      if (args.data.length > 0) {
        ...
        const stmt = await db.prepare(
          `INSERT INTO export_data VALUES (${placeholders})`,
        );
        for (const row of args.data) {
          const values = args.columns.map((col) => {
            const val = (row as Record<string, unknown>)[col.source];
      ...
      await db.run(
        `COPY export_data TO '${parquetPath}' (FORMAT PARQUET, COMPRESSION ZSTD);`,
      );
      await db.close();
...
export const snapshotChunkToS3 = action({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.data.length > 0) {
      const placeholders = args.columns.map(() => "?").join(", ");
      const stmt = await db.prepare(
        `INSERT INTO chunk_data VALUES (${placeholders})`,
      );
      for (const row of args.data) {
        const values = args.columns.map((col) => {
          const val = (row as Record<string, unknown>)[col.source];
    ...
    await db.run(
      `COPY chunk_data TO '${parquetPath}' (FORMAT PARQUET, COMPRESSION ZSTD);`,
    );
    await db.close();
...
export const queryDuckDB = action({
  ...
  handler: async (ctx, args) => {
    ...
    for (const table of args.table_s3_paths) {
      await db.run(
        `CREATE VIEW "${table.table_name}" AS SELECT * FROM read_parquet('s3://${args.s3_bucket}/${table.s3_path}');`,
      );
    }
    ...
    const columns =
      result.length > 0 ? Object.keys(result[0] as object) : [];
